        ...
        if (url.includes(baseUrl)) {
            ...
            await $(panelBody).on('DOMNodeInserted', function () {
                ...
                if (count === 0) {
                    ...
                    // get resource
                    resources = Array.from(trs).filter(tr => {
                        const _tr = $(tr).find('td');
                        return _tr[7].innerText;
                    }).map(tr => {
                        const _tr = $(tr).find('td');
                        ...
                        return {
                            ...
                            _delete: () => deleteLink.click()
                        }
                    });

                    console.log(resources[0]);
                    resources[0]._delete();
                    // when modal show click to button with attr data-bb-handler="confirm"
                    const btn = $('button[data-bb-handler="confirm"]');
                    console.log("click");
                    btn.click();
                }
            });
